...
class ChatWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        self.header_label = QLabel(f"💬 Чат с {self.contact_name}")
        self.header_label.setStyleSheet("font-weight: bold;")
        self.lock_icon = QLabel("🔓")
        self.lock_icon.setToolTip("OMEMO шифрование не активно")
        self.lock_icon.setStyleSheet("font-size: 14px;")
        header_layout.addWidget(self.header_label)
        header_layout.addStretch()
        header_layout.addWidget(self.lock_icon)
        header_widget.setStyleSheet("background-color: #333; border-bottom: 1px solid #444;")
    ...
    def set_encryption_status(self, active: bool):
        self.encryption_active = active
        self.lock_icon.setText("🔒" if active else "🔓")
        self.lock_icon.setToolTip("OMEMO шифрование активно" if active else "OMEMO шифрование не активно")
...
class MainWindow(QMainWindow):
    ...
    def login(self, username, password, server, port):
        ...
        self.xmpp_thread.omemo_available.connect(self.on_omemo_available)
    ...
    def on_omemo_available(self, jid, available):
        if jid in self.chat_windows:
            self.chat_windows[jid].set_encryption_status(available)
    ...
    def show_about(self):
        QMessageBox.about(self, "О программе",
            ...
            "<p>Поддержка OMEMO шифрования и порта 5223</p>"
